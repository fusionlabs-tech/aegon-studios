    ...
    <div className='container mx-auto px-6 md:px-16'>
     <div className='flex flex-col md:flex-row justify-between items-end gap-12'>
      <div className='max-w-3xl'>
       ...
       <h1 className='text-[clamp(4rem,12vw,10rem)] leading-[0.8] font-display font-bold mb-8'>
        PROJECT <br />
        <span className='text-transparent stroke-text opacity-50'>ARCHIVE</span>
       </h1>
       <p className='text-xl md:text-2xl font-sans opacity-50 leading-relaxed max-w-xl'>
        A comprehensive record of visual storytelling, commercial production,
       ...
       {categories.map((cat) => (
        <button
         ...
         className={`text-[10px] font-bold tracking-[0.3em] uppercase transition-all relative ${
          filter === cat ? 'text-foreground' : 'opacity-40 hover:opacity-100'
         }`}
        >
         {cat}
         {filter === cat && (
          <motion.div
           ...
           className='absolute -bottom-4 left-0 right-0 h-px bg-foreground'
          />
         )}
        </button>
       ))}
   ...
   <section className='py-32'>
    <div className='container mx-auto px-6 md:px-16'>
      ...
      <AnimatePresence mode='popLayout'>
       {paginatedItems.map((item, idx) => (
        <Link key={item.id} to={`/project/${item.id}`}>
         ...
         >
          <div className='overflow-hidden bg-card aspect-[4/5] mb-8 relative border border-border'>
           <img
            src={item.image}
            alt={item.title}
          ...
          <div className='space-y-4 border-t border-border pt-6'>
           <div className='flex justify-between items-start'>
            <h3 className='text-3xl font-display font-medium tracking-tight transition-colors uppercase'>
             {item.title}
            </h3>
            <span className='text-xl font-display opacity-20 group-hover:opacity-100 transition-opacity'>
             /{item.id.toString().padStart(2, '0')}
            </span>
           </div>

           <p className='opacity-40 text-sm font-sans leading-relaxed line-clamp-2'>
            {item.description}
           </p>
     ...
     {filteredItems.length === 0 && (
      <div className='py-40 text-center border border-border border-dashed'>
       <p className='text-2xl opacity-20 font-display font-bold tracking-widest uppercase'>
        No projects found.
       </p>
      </div>
     )}
     ...
     {totalPages > 1 && (
      <div className='mt-40 border-t border-border pt-12 flex flex-col md:flex-row justify-between items-center gap-8'>
        ...
        <div className='flex gap-2'>
         {[...Array(totalPages)].map((_, i) => (
          <button
           ...
           className={`w-12 h-12 text-[10px] font-bold tracking-widest flex items-center justify-center transition-all border border-border rounded-full ${
            currentPage === i + 1
       ...
       <p className='text-[10px] font-bold tracking-[0.4em] opacity-40 uppercase'>
