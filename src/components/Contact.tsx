...
export function Contact({
 ...
 return (
  <section
   id='contact'
   className={`relative bg-background border-t border-border ${showHeader ? 'py-24 md:py-32' : 'py-16 md:py-24'}`}
  >
   {/* Background elements - Always show for ambience */}
    ...
    <div className='grid lg:grid-cols-2 gap-16 lg:gap-24'>
     ...
     >
      {showHeader && (
       <>
        ...
        <p className='text-xl opacity-60 mb-12 leading-relaxed max-w-md'>
         ...
         possibilities, we're here to help you tell your story.
        </p>
       </>
      )}
      ...
      <div className='flex items-center gap-6'>
       <div className='text-xs font-bold tracking-widest opacity-40 uppercase'>
        Follow
       </div>
     ...
     <motion.div
      ...
      className='bg-card border border-border p-8 md:p-12 rounded-3xl shadow-xl'
     >
      <form onSubmit={handleSubmit} className='space-y-6'>
       <div className='grid md:grid-cols-2 gap-6'>
        <div className='space-y-2'>
         <label className='text-xs font-bold tracking-widest opacity-40 uppercase ml-1'>
          Full Name
         </label>
         <Input
          required
          placeholder='John Doe'
          className='bg-foreground/5 border-border focus:border-foreground/40 h-14 px-5 rounded-xl transition-all duration-300 focus:bg-background'
         />
        </div>
        <div className='space-y-2'>
         <label className='text-xs font-bold tracking-widest opacity-40 uppercase ml-1'>
       ...
       <div className='space-y-2'>
        ...
        <Select>
         <SelectTrigger className='bg-foreground/5 border-border focus:border-foreground/40 h-14 px-5 rounded-xl transition-all duration-300 focus:bg-background'>
       ...
       <div className='space-y-2'>
        ...
        <Textarea
         ...
         className='bg-foreground/5 border-border focus:border-foreground/40 min-h-[150px] p-5 rounded-xl resize-none transition-all duration-300 focus:bg-background'
        />
       </div>
       ...
       >
        <span
         className={`flex items-center justify-center gap-3 transition-transform duration-500 ${isSubmitting ? '-translate-y-full' : 'translate-y-0'}`}
        >
         SEND MESSAGE
         <PaperPlaneTilt
          weight='bold'
        ...
        <span
         className={`absolute inset-0 flex items-center justify-center gap-2 transition-transform duration-500 ${isSubmitting ? 'translate-y-0' : 'translate-y-full'}`}
        >
         SENDING...
        </span>
       </Button>
